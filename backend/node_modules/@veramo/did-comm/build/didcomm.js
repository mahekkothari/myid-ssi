...
import { asArray, bytesToUtf8String, decodeJoseBlob, dereferenceDidKeys, encodeJoseBlob, extractPublicKeyHex, hexToBytes, isDefined, mapIdentifierKeysToDoc, resolveDidOrThrow, stringToUtf8Bytes, } from '@veramo/utils';
...
export class DIDComm {
    ...
    async packDIDCommMessageJWS(args, context) {
        ...
        const senderKeys = await mapIdentifierKeysToDoc(managedSender, 'authentication', context, args.resolutionOptions);
    ...
    async packDIDCommMessageJWE(args, context) {
        ...
        if (args.packing === 'authcrypt') {
            ...
            const senderKeys = await mapIdentifierKeysToDoc(sender, 'keyAgreement', context, args.resolutionOptions);
        ...
        async function computeRecipients(to, resolutionOptions) {
            ...
            const didDocument = await resolveDidOrThrow(to, context, resolutionOptions);
    ...
    async unpackDIDCommMessageJWE({ jwe }, context, resolutionOptions) {
        ...
        let senderKeyBytes = await extractSenderEncryptionKey(jwe, context, resolutionOptions);
